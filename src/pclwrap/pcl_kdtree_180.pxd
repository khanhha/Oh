...
# class KdTree
cdef extern from "pcl/kdtree/kdtree.h" namespace "pcl":
...
cdef extern from "pcl/kdtree/kdtree_flann.h" namespace "pcl":
    cdef cppclass KdTreeFLANN[T]:
        ...
        void setNormalCloud(shared_ptr[cpp.PointCloud[cpp.Normal]])

        void setMaxPointsPerLeaf(size_t npoins)

        void setNormalThreshold(float thres)
        ...
        int nearestKSearch (const T &point, int k, vector[int] &k_indices, vector[float] &k_sqr_distances)
        ...
        int radiusSearch (const T &point, double radius, vector[int] &k_indices, vector[float] &k_sqr_distances, unsigned int max_nn)
        ...
        int getAllLeafNodesBoundingBox(vector[float] &bmin, vector[float] &bmax) const
        int getNodesBoundingBoxAtMaxDepth(int expected_depth, vector[float] &bmin, vector[float] &bmax, vector[int] &depths) const
        int getNodesBoundingBoxAtDepth(int expected_depth, vector[float] &bmin, vector[float] &bmax) const
        int getLeafPointIndices(const T &point, vector[int] &indices) const;
