...
cdef extern from "pcl/octree/octree_pointcloud.h" namespace "pcl::octree":
    cdef cppclass OctreePointCloud[PointT, LeafT, BranchT, OctreeT]:
        ...
        #  */
        void genVoxelBoundsFromOctreeKey (const OctreeKey key_arg, unsigned int tree_depth_arg, eig.Vector3f min_pt, eig.Vector3f max_pt) const
        ...
        int getOctreeKeysAtMaxDepth(unsigned int depth, vector[OctreeKey] keys, vector[int] depths) const

        int getOctreeKeysAtDepth(unsigned int depth, vector[OctreeKey] keys, vector[int] depths) const

        int getAllLeafKeys(vector[OctreeKey] keys, vector[int] depths) const
...
cdef extern from "pcl/octree/octree_search.h" namespace "pcl::octree":
    cdef cppclass OctreePointCloudSearch[PointT](OctreePointCloud[PointT, OctreeContainerPointIndices_t, OctreeContainerEmpty_t, OctreeBase_OctreeContainerPointIndices_t]):
        ...
        int nearestKSearch (PointT, unsigned int, vector[int], vector[float])
        ...
        int boxSearch (const PointT &min_pt, const PointT &max_pt, vector[int] &k_indices)
