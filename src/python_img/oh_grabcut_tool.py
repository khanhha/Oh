...
if __name__ == '__main__':
    ...
    out_dir = "D:\Projects\Oh\data\images\\trainim_LL_0428\\test\\silhouette\\"
    ...
    if len(file_paths) == 0:
        print('no file in the director %s' % dir)
        exit(1)

    img = cv.imread(file_paths[cur_file_idx])
    img = scale_image(img,0.25)

    #img = cv.fastNlMeansDenoisingColored(img, None, 10, 10, 7, 21)
    #img = cv.GaussianBlur(img, (25,25), 15)
    ...
    while(1):
        ...
        if k == 27:         # esc to exit
        ...
        elif k == ord('d'): # save image
            bar = np.zeros((img.shape[0],5,3),np.uint8)
            res = np.hstack((img2,bar,img,bar,output))
            img_name = os.path.basename(file_paths[cur_file_idx])
            cv.imwrite(os.path.join(out_dir, img_name),res)
            print(" Result saved as image \n")

        elif k == ord('s'): # save image
            img_name = os.path.basename(file_paths[cur_file_idx])
            cv.imwrite(os.path.join(out_dir, img_name),output)
            print(" Result saved as image %s\n" % os.path.join(out_dir, img_name))
            ...
            cur_file_idx += 1
            if cur_file_idx < len(file_paths):
                print("process file {0} \t {1}".format(cur_file_idx, file_paths[cur_file_idx]))
                img2 = cv.imread(file_paths[cur_file_idx])
                img2 = scale_image(img2, 0.25)
                rect = (0, 0, 1, 1)
                drawing = False
                rectangle = False
                rect_or_mask = 100
